...
Module CommonFunctions
    ...
    Public Function oReadXMLFile(ByVal ofile As String, ByVal nodename As String) As String

        Using reader As XmlReader = XmlReader.Create(ofile)
            While reader.Read()
                ' Check for start elements.
                If reader.IsStartElement() Then
                    If reader.Name = nodename Then
                        ' get Text data.
                        If reader.Read() Then
                            Dim strPulledValue As String = reader.Value.ToString.Trim
                            reader.Close()
                            Return strPulledValue
                        End If
    ...
    End Sub
        ...
        Public Function IsValidSiteCollectionUrl(ByVal strSiteCollectionUrl As String) As Boolean


            Dim bValidSiteCol As Boolean = False
            'Dim oSPSite As Microsoft.SharePoint.SPSite = Nothing
        ...
        Catch ex As Exception
            MessageBox.Show("ERROR connecting to sharepoint->" + ex.Message)
            Return False
            End Try
        ...
        Public Function oConnectToSolidEdge(ByVal blnAppVisibility As Boolean, ByVal blnDisplayAlerts As Boolean, Optional ByVal oStopSE As String = "NO") As Boolean
            ...
            Else
                ...
                Try  'close down and restart solid max has been reached
                    Do While objSEApp.Documents.Count <> 0
                        objSEApp.ActiveDocument.close(True)
                        objSEApp.DoIdle()
                    Loop
                    objSEApp.Quit()
                    objSEApp.DoIdle()
                ...
                'SE should not be running then start it
                Try
                    objSEApp = Activator.CreateInstance(objSEType)
                    objSEApp.DisplayAlerts = blnDisplayAlerts
                    objSEApp.Visible = blnAppVisibility
        ...
        'pass in the full path including filename or just the filename
        Public Function oIsSolidEdgeAssemblyFile(ByVal Filename As String) As Boolean
            Dim sExtension As String = Nothing
            Dim bPart As Boolean = False
...
skip:
            ...
            Return Nothing
        End Function
    Public Function oReadFileGetURLs(ByVal oFilename As String) As String
        Dim strtmpTCNumber As String = String.Empty
        Dim strtmpTCREvision As String = String.Empty
        ...
        Try
            ...
            For Each strData In arrData
                ...
                arrayUrlOfFilesToCheckIn.Add(arrStrDataSplit(1))
...
skip:
        ...
        Return Nothing
    End Function
        Public Function oReadFile(ByVal oFilename As String) As String
            Dim strtmpTCNumber As String = String.Empty
            Dim strtmpTCREvision As String = String.Empty
            Dim lngCtr As Long = 0
            ...
            Try
                ...
                For Each strData In arrData
                    ...
                    Dim arrStrDataSplit As String() = strData.Split(",")
                    strtmpTCNumber = arrStrDataSplit(0)
                    strtmpTCREvision = arrStrDataSplit(1)
                    arrayTCItems.Add(strtmpTCNumber)
                    arrayTCRevisions.Add(strtmpTCREvision)
